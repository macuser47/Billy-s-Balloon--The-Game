...
namespace CastleWarrior
{
    class PhysicsObject
    {
        ...
        List<Rectangle> collidableObjectList;
        ...
        public Vector2 Update(GraphicsDevice graphicsDevice, Vector2 Position, bool isGravity, float jumpFactor, float rightVelocity, float leftVelocity, float upVelocity, float downVelocity, float textWidth, float textHeight, List<Rectangle> collidableObjectList, bool applyPlayerGravityXYMovement)
        {
            ...
            foreach (Rectangle obj in collidableObjectList)
            {
            ...
            if ((Keyboard.GetState().IsKeyDown(Keys.Up) || Keyboard.GetState().IsKeyDown(Keys.Space)) && gravity == 0 && isGravity/*&& loopCount == 1*/)
            {
                gravity = -jumpFactor;
            }
